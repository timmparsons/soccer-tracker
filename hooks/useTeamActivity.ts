...
// How far back the feed looks for activity from any user on the team.
const FEED_WINDOW_DAYS = 60;

async function fetchActivityItems(limit: number): Promise<TeamActivityItem[]> {
  const windowAgoDate = getLocalDate(
    new Date(Date.now() - FEED_WINDOW_DAYS * 24 * 60 * 60 * 1000),
  );
  const windowAgo = new Date(
    Date.now() - FEED_WINDOW_DAYS * 24 * 60 * 60 * 1000,
  );

  // Fetch sessions from the window across all users
  const { data: sessions } = await supabase
    .from('daily_sessions')
    .select(
      'user_id, date, touches_logged, drill_id, juggle_count, created_at, is_game_speed, duration_minutes, drills(name)',
    )
    .gte('date', windowAgoDate)
  ...
  const { data: rawBadgeEarns } = await (supabase as any)
    ...
    .gte('earned_at', windowAgo.toISOString())
  ...
  // Streak needs full session history, not the window above — fetch it
  // only for the handful of users who actually made the feed.
...
export function useActivityFeed(limit = 7) {
  ...
  return useQuery({
    ...
    queryFn: () => fetchActivityItems(limit),
  });
}
